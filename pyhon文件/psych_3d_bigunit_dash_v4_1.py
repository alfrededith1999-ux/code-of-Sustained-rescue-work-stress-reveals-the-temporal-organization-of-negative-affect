...
def sql_quote(v):
    """把值安全地包成 SQL 单引号字符串： 'abc' -> 'abc'，内部单引号变成 '' """
...
def make_coalesce_expr(cols_list):
    """生成 SQLite 表达式：COALESCE(NULLIF(TRIM(COALESCE(col,'')),''), ...)"""
    if not cols_list:
        return "''"
    parts = []
    for c in cols_list:
...
def main():
    ...
    ap.add_argument("--max_points", type=int, default=12000)
    ap.add_argument("--schema_sample_n", type=int, default=2000)
    ...
    if not wave_col:
        raise RuntimeError("表/视图中必须有 WAVE 列（如 '24Q1'）。")
    ...
    if args.bigunit_col:
        if args.bigunit_col not in cols:
            raise RuntimeError(f"--bigunit_col 列不存在：{args.bigunit_col}")
        big_candidates = [args.bigunit_col]

    big_expr = make_coalesce_expr(big_candidates)
    outcome_cols = detect_outcomes(cols)
    label_cols = detect_labels(cols)

    # sample for feature sets
    sample_df = read_sql(args.db, f"SELECT * FROM {args.table} LIMIT {int(args.schema_sample_n)}")

    exclude_cols = {person_col, wave_col}
    if name_col:
    ...
    app.layout = html.Div([
        ...
        html.Div([
            ...
            html.Div([
                ...
                html.Div("提示：labels=0 也能跑；只是没Y_列可选。", style={"fontSize": "12px"})
    ...
    def render_tab(tab):
                    ...
                    html.Div([
                        ...
                        dcc.Dropdown(
                            ...
                            options=[
                                ...
                                {"label": "嵌入轨迹（每波次映射到3D嵌入空间）", "value": "EMBED"},
                            ],
                            value="METRIC",
                            clearable=False
                        )
        ...
        return html.Div([
            ...
            html.Div([html.Div("T1"), dcc.Slider(id="thr1", min=0, max=40, step=1, value=10,
                                                tooltip={"placement": "bottom", "always_visible": True})],
                     style={"marginBottom": "8px"}),

            html.Div([html.Div("T2"), dcc.Slider(id="thr2", min=0, max=60, step=1, value=15,
                                                tooltip={"placement": "bottom", "always_visible": True})],
                     style={"marginBottom": "8px"}),

            html.Div([html.Div("干预强度（+越大越有利）"),
                      dcc.Slider(id="interv", min=0, max=10, step=1, value=3,
                                 tooltip={"placement": "bottom", "always_visible": True})],
                     style={"marginBottom": "8px"}),

            html.Div([html.Div("压力变化（+越大越不利）"),
                      dcc.Slider(id="stress", min=-10, max=10, step=1, value=2,
                                 tooltip={"placement": "bottom", "always_visible": True})],
                     style={"marginBottom": "8px"}),

            html.Div([html.Div("支持/资源变化（+越大越有利）"),
    ...
    def fetch_subset(waves, bigunits, need_cols, limit=None):
        wh = []
        if waves:
            ws = ",".join([sql_quote(w) for w in waves])
            wh.append(f"\"{wave_col}\" IN ({ws})")
        if bigunits:
            bu = ",".join([sql_quote(b) for b in bigunits])
    ...
    def update_group(n, outcome_col, label_col, waves, bigunits, mode, method, fset, max_points):
        ...
        key = ("GROUP", tuple(sorted(waves)) if waves else ("ALL",),
               tuple(sorted(bigunits)) if bigunits else ("ALL",), method, fset)
        ...
        if mode == "CENTROID":
            ...
            fig.add_trace(go.Scatter3d(
                ...
                mode="markers",
                marker=dict(
                    size=np.clip(cen["n_rows"].to_numpy() / 80.0, 6, 22),
            ...
            title = f"群体3D（单位质心）| method={meta.get('method')} | units={len(cen)}"
        else:
            hover = []
            ...
            title = f"群体3D（人员点云）| method={meta.get('method')} | n_points={len(dfg)}"
        ...
        info = []
        info.append(f"大单位列优先级={big_candidates}")
    ...
    def find_person(n, bigunit, waves, qname, qphone4, qpid):
        ...
        for _, r in dfx.iterrows():
            ...
            nm = _trim(r.get(name_col)) if name_col else ""
            ph = _trim(r.get(phone_col)) if phone_col else ""
            opts.append({"label": f"{pid} | {nm} | {ph}", "value": pid})

        info = f"单位={bigunit}\n匹配到 {len(opts)} 人（最多60）\n主键={person_col} | name={name_col} | phone={phone_col}"
    ...
    def update_ind(pid, outcome_col, label_col, waves, traj_mode, bigunit, fset):
        ...
        if traj_mode == "EMBED":
            ...
            dfg = fetch_subset(waves, [bu] if bu else ([bigunit] if bigunit else []),
                               [person_col, wave_col] + feats, limit=None)
            if len(dfg) > 6000:
            ...
            if key in EMBED_CACHE:
                ...
                _, meta, transformer = fit_embedder(Xg, method="PCA")
                EMBED_CACHE[key] = {"transformer": transformer, "meta": meta, "feats": feats}
            ...
            fig.add_trace(go.Scatter3d(
                ...
                marker=dict(size=5),
                line=dict(width=4),
                text=[f"WAVE={w}<br>{outcome_col}={y}"
                      for w, y in zip(dfi[wave_col], dfi[outcome_col])],
                hoverinfo="text",
    ...
    def make_ending(n, bigunit, pid, outcome_col, rule, t1, t2, interv, stress, support, waves):
        pid = (pid or "").strip()
        if not pid:
            return f"请粘贴一个人的ID（{person_col}）。建议到“个体”页查到后复制过来。"
        if not outcome_col:
            return "请先在顶部选择 outcome_col。"
        ...
        if risk == "高风险" and interv <= 2:
            ...
            ending = "结局：仍高风险但出现止跌迹象（强干预抵消部分压力）。建议：维持干预并增强支持。"
        ...
        return "\n".join([
            f"人：{person_col}={pid}",
            f"单位：{bu}",
            f"当前波次：{w0} | 当前分数 {outcome_col}={y0:.2f}",
            f"交互设定：干预={interv} | 压力变化={stress} | 支持变化={support}",
            f"模拟后分数：{outcome_col}_SIM={y_sim:.2f}",
            f"风险判定：{risk}（规则={rule}，T1={t1}, T2={t2}）",
            ending,
            "",
            "注：这是演示级规则模拟，不等价因果推断；要严谨版可接你训练的模型做what-if。"
        ])
